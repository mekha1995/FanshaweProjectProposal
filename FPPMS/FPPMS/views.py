        ...
        if request.method=="POST" :
            # request.FILES['document'].is_valid():
                ...
                if saveserialize.is_valid():
                    ...
                    messages.info(request, 'Proposals submitted successfully!')
...
def insertproposal(request):
    if request.method=="POST":
        ...
        if saveserialize.is_valid():
            ...
            read=requests.post('https://localhost/Insertproposal',json=data,headers=headers)
            if(read):
